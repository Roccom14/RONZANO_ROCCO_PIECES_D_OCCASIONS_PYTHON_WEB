...
class GestionStuff:
    ...
    def stuff_afficher_data(self):
        try:
            ...
            with MaBaseDeDonnee().connexion_bd.cursor() as mc_afficher:
                ...
                print("data_user ", data_stuff, " Type : ", type(data_stuff))
    ...
    def add_stuff_data(self, valeurs_insertion_dictionnaire):
        try:
            ...
            strsql_insert_stuff = """INSERT INTO t_stuff (id_stuff, name_stuff, description_stuff, price_stuff, type_stuff, quantity_stuff, firstname_user, lastname_user, state_stuff, type_payment, date_add_stuff, date_bought_stuff) 
                                    VALUES (NULL, %(value_name_stuff)s, %(value_description_stuff)s, %(value_price_stuff)s, %(value_type_stuff)s, %(value_quantity_stuff)s, %(value_firstname_user)s, %(value_state_stuff)s, %(value_type_payment)s, %(value_date_add_stuff)s, %(value_date_bought_stuff)s);"""
            # Du fait de l'utilisation des "context managers" on accède au curseur grâce au "with".
    ...
    def edit_stuff_data(self, valeur_id_dictionnaire):
            try:
                ...
                str_sql_id_stuff = """SELECT id_stuff, name_stuff, description_stuff, price_stuff, type_stuff, quantity_stuff, firstname_user, lastname_user, state_stuff, type_payment, date_add_stuff, date_bought_stuff FROM t_stuff 
                                        INNER JOIN t_user ON t_stuff.fk_user = t_user.id_user 
                                        INNER JOIN t_state_stuff ON t_stuff.fk_state_stuff = t_state_stuff.id_state_stuff 
